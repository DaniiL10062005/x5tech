...
internal class DetectBankByBinUseCase {

    operator fun invoke(input: String): String? {
        val digits = input.filter(Char::isDigit)
        val bin = digits.take(BIN_LENGTH)
        if (bin.length < BIN_LENGTH) {
            return null
        }

        val numericBin = bin.toIntOrNull() ?: return null

        return bankRanges.firstNotNullOfOrNull { bankRange ->
            bankRange.bankName.takeIf { numericBin in bankRange.range }
        }
    }
